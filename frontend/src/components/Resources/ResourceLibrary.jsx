...
const normalizeResourceResponse = (response) => {
  ...
  // Case 1: Server returned array directly (shouldn't happen but handle it)
  if (Array.isArray(payload)) {
    return { data: payload, pagination: {} };
  }
  
  // Case 2: Server returned { data: [...], pagination: {...} }
...
const ResourceLibrary = () => {
  ...
  useEffect(() => {
    const fetchResources = async () => {
      ...
      try {
        ...
        const hasMoreVal = (pagination.page || 1) < (pagination.pages || 1);
  ...
  const getId = (resource) => resource?._id || resource?.id;

  const handleEditResource = (resource) => {
    setEditingResource(resource);
    setEditingId(getId(resource));
    setShowForm(true);
  };
  ...
  const handleDeleteResource = async (id) => {
    ...
    try {
      ...
      setResources(prev => prev.filter(r => getId(r) !== id));
  ...
  const handleFormSubmit = async (formData) => {
    try {
      ...
      setEditingId(null);
      setEditingResource(null);
      // Refetch resources by clearing search to trigger the useEffect
      setSearchQuery('');
    } catch (err) {
  ...
  const handlePinResource = async (id, isPinned) => {
    try {
      ...
      setResources(prev => prev.map(r => {
        const rid = getId(r);
        return rid === id ? { ...r, isPinned: !r.isPinned } : r;
      }));
    } catch (err) {
  ...
  const handleLoadMore = async (nextPage) => {
    ...
    try {
      ...
      const response = await api.get(endpoint, { params: { page: nextPage } });
      const { data, pagination } = normalizeResourceResponse(response);
        ...
        >
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="px-5 py-3 rounded-xl bg-gradient-to-r from-indigo-500 to-cyan-500 text-white font-semibold shadow-md hover:shadow-lg transition-transform hover:scale-[1.02]"
            >
            ...
            <button
              ...
              className="px-4 py-3 rounded-xl bg-white text-slate-700 border border-slate-200 shadow-sm hover:shadow-md transition-transform hover:scale-[1.02]"
          ...
          {loading && resources.length === 0 ? (
            ...
            >
              {resources.map((resource, index) => (
                <ResourceCard
                  key={getId(resource) || index}
                  resource={resource}
                  index={index}
                  viewMode={viewMode}
                  onEdit={() => handleEditResource(resource)}
                  onDelete={() => handleDeleteResource(getId(resource))}
                  onPin={() => handlePinResource(getId(resource), resource.isPinned)}
                />
              ))}
            </div>
          ) : null}
